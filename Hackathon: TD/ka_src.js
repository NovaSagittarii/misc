...
var Polygon = function(x, y, radius, npoints){
    var angle = degrees(TWO_PI / npoints);
    beginShape();
    for (var a = 0; a < degrees(TWO_PI); a += angle) {
...
Turret.prototype.draw = function(x, y) {
    for(var k = 0; k < enemies.length; k ++){
        if(dist(enemies[k].x, enemies[k].y, x, y) < tdata.range[this.id]*1.5){
            ...
            if(this.reload > tdata.rof[this.id]){
                this.reload = 0;
                bullets.push(new Bullet(x, y, TH + random(-tdata.accu[this.id], tdata.accu[this.id]), tdata.speed[this.id], tdata.dmg[this.id], tdata.pierce[this.id], tdata.range[this.id]));
            }
            break;
        }
    }
    ...
    popMatrix();
    fill(0, 0, 0, 10);
    ellipse(0, 0, tdata.range[this.id]*3, tdata.range[this.id]*3);
};
        ...
        case 3:
            ...
            for(var i = 0; i < gmap.length; i ++){
                for(var j = 0; j < gmap[i].length; j ++){
                    ...
                    if(isNaN(gmap[i][j])){
                        ...
                        if(abs(mouseY-(i*50+25)) < 25 && abs(mouseX-(j*50+25)) < 25 && buildMode === null && gmap[i][j].id < 4){
                            fill(250, 170, 170);
            ...
            for(var i = 0; i < 6; i ++){
                if(i < 4){
                    ...
                    fill(130, 130, 130);
                    if(i === 4){
                        if(waveAmt < waves[wave].amt && enemies.length > 0){
                            fill(250, 170, 170);
                        }else{
                            fill(150, 255, 245);
                        }
                ...
                if(abs(mouseX - 670) < 100 && abs(mouseY - (i*75)-180) < 30){
                    ...
                    if(i === 4 && waveAmt >= waves[wave].amt && enemies.length < 1){
                        fill(255, 255, 255, 150);
                        rect(450, 180 + i*75, 200, 60);
                        fill(0, 0, 0);
